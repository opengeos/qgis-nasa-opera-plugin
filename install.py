#!/usr/bin/env python3
"""
Cross-platform installation script for NASA OPERA QGIS Plugin.
...
def install_plugin(
    source_dir: Path, plugin_dir: Path, plugin_name: str = "nasa_opera"
) -> bool:
...
def remove_plugin(plugin_dir: Path, plugin_name: str = "nasa_opera") -> bool:
...
def main():
    parser = argparse.ArgumentParser(
        description="Install or remove NASA OPERA QGIS Plugin"
    )
    parser.add_argument(
        "--remove",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default="nasa_opera",
        help="Plugin folder name in QGIS plugins directory (default: nasa_opera)",
    )
    args = parser.parse_args()

    # Get script directory
    script_dir = Path(__file__).parent.resolve()
    source_dir = script_dir / "nasa_opera"
    ...
    if args.remove:
        ...
        if success:
            ...
            print(f"  3. Enable 'NASA OPERA'")
            print()
            print("Prerequisites:")
            print("  Make sure you have the following Python packages installed:")
            print("    pip install earthaccess geopandas shapely pandas")
            print()
            print("  You also need a NASA Earthdata account:")
